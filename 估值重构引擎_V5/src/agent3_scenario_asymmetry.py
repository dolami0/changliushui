...
**赋 PE/PS 的三步法（baseline 已做前两步——你用第三步）**:
1. **参照系已在 baseline 中**: 投资地图的「产业位置」维度列出了可比公司+市占率+竞争位势，「量化锚点」维度给出了可比PE/PS中位数和历史PE band。**不需要重新搜索可比公司**——直接用 baseline 的数据。
2. **读 baseline 的数**: 可比公司当前交易的 PE/PS 中位数是多少？历史稳态区间是多少？你可以在 baseline 量化锚点中找到这些数据。
3. **对标赋参**: 你的 bear/base/bull PE/PS 必须落在这个参照系的合理区间内——可以有溢价/折价,但必须有理由。如果某情景的 PE/PS 超出 baseline 参照系范围,必须写"PE突破论证"或"PS突破论证"。

**PS 的参照框架**:
- **参照系来源**: baseline「量化锚点」中的可比 PE/PS 中位数 + 「产业位置」中的竞争位势。不是凭印象回忆——baseline 里有具体数据。
- **锚定方法**: 可比公司 PS/PE 的当前时点值可能受板块情绪影响——结合历史稳态区间判断。baseline 量化锚点中可能包含历史 PE band。
- **Bull PS** = 行业领导者在稳态下的 PS。不是泡沫峰值。baseline 参照系中最好的公司在非泡沫期交易的水平。
- **Base PS** = 中等偏上公司在稳态下的 PS。baseline 可比中位数 ± 合理溢价。
...
**PE 的参照框架**:
- 与 PS 相同: baseline「量化锚点」中的可比 PE 中位数是锚定起点。火山数据中的当前 PE 仅供参考。
- **Bull PE** = 行业领导者在稳态下的 PE。baseline 参照系中盈利质量最高的可比公司。
- **Bear PE** = 行业周期底部的 PE。baseline 历史 PE band 的底部区域。
...
**赋参数后必须做 baseline 锚点交叉校验:**

赋完三情景 PE/PS/PB 后，回看 baseline「量化锚点」中给出的可比中位数和历史 PE band:
- 你的 **base PE/PS** 与可比中位数的偏差应在合理范围（±30%）内。超出范围→必须在 scenario_narrative 中写一句理由（溢价因何？折价因何？）
- 你的 **bull PE/PS** 不应超过可比参照系中最好的公司在非泡沫期的交易峰值。如果你找不到一家参照系公司曾经交易在这个水平→说明你的 bull 倍数假设缺乏市场交易的历史支撑
- 你的 **bear PE/PS** 应接近或低于可比公司在周期底部的交易水平
- 如果三情景的倍数都系统性高于/低于参照系→你可能在用缩放而非锚定。重新走三步法
