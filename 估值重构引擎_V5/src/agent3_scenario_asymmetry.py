...
**Baseline（地基——参数的下限约束和合理范围）**:
- 财务基线（维度二）: 当前 ROIC/毛利率/净利率/营收——参数的**推演起点**。事件还没发生前公司就是这个水平，参数不能无视这个起点。
- 产业位置（维度三）: 可比公司 PE/PS/PB 中位数——参数的**合理范围**。你的 bear/base/bull 参数是否在这个范围内？超出范围 → 必须有"突破论证"。
- 量化锚点（维度六）: 当前产能/价格/市占率——事件冲击前的**硬数字**。事件说产能从 2700 吨→5000 吨，但 baseline 说当前就是 2700 吨——起点对得上才算锚住了。
- 脆弱点（维度五）: 什么假设最容易崩塌——约束 bear 的下跌空间和 bull 的上限。脆弱度高 → bear 概率不能太低，bull 参数不能太激进。

**事件锚（变数——参数的变化方向和幅度）**:
- 事件中的新价格/新产能/新客户/新技术——从 baseline 起点往哪个方向移、移多少。
- 按来源置信度打折: 公司公告 <10%，专家纪要 10-30%，分析师测算 20-40%。
- 打折必须显式标注: 锚点原文→来源→置信度→打折幅度→打折后数字→理由。不允许综合一句"更保守"覆盖所有打折。

**两者的关系**: Baseline 是重力——参数不能飞到物理不可能的水平（如 ROIC 从 -1.5%→25% 需要具体事件机制支撑，不能纯假设）。事件是推力——参数不能停在 baseline 不动（如果事件说涨价 30-50%，毛利率必须上移）。参数的最终位置 = baseline 起点 + 事件推力 × 置信度打折。推力和重力都要在 reasoning_trace 中显式写明。
