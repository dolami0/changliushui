...
**前置: 风险映射** — 逐条阅读逆向风险。每条风险主要约束哪个情景？在写剧本之前先明确:
- 涉及已发生负面事实的风险（如内部人减持、客户集中度高、产能不足）→ 同时约束 base 和 bull。base 参数不能建立在"这些问题不存在"的假设上；bull 需要说明这些问题如何被克服。
...
赋参数的起点是 3d 的因果剧本和风险映射。在动手赋参数之前，先确认两件事:
