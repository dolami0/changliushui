...
例: "事件讨论 30 倍 PE 隐含 450 亿市值，反映的是分析师对电子级 PI 盈利弹性的预期。路由判 revenue 锚，因为 ROIC -1.5% 不配 PE。我的 base PS=8x 对应: 全球 PI 缺口 1 万吨+国产唯一标的+涨价 30-50%→应享受稀缺溢价。8x PS 在 A 股半导体材料可比区间 6-12x 内，反映了事件催化但未到 bull 极端值。"
...
**PS 的参照框架**:
- **锚定方法**: 凭行业知识判断同细分赛道的 A 股公司在对应生命周期阶段的估值水平。可比公司 PS/PE 是当前时点值——可能整个板块都在泡沫或恐慌中——仅供参考，不能直接照搬。
- **Bull PS** = 行业领导者在成长阶段的 PS（若公司处于成长阶段）或稳态下的 PS（若已成熟）。不是泡沫峰值。
- **Base PS** = 反映公司**当前生命周期位置**的合理PS——不是稳态PS。公司处于高增长阶段（CAGR>30%、新技术刚验证、订单爆发）→ PS 应取成长阶段对应水平，不能机械对标稳态公司。事件催化（验证突破、涨价东风、订单翻倍）必须体现在 PS 溢价中——增速之外，技术壁垒和行业东风也值钱。
- **Bear PS** = commoditized 参与者或周期底部的 PS。不是危机恐慌低点——是"故事证伪后,市场在正常情况下持续交易该股票的底部区间"。
- **可以突破参照系——但必须输出理由**: 若 PS 超出可比公司参照系范围，在 reasoning_trace 中单独写一条"PS突破论证"。事件催化充分时，突破参照系是合理的——不突破才需要解释为什么催化没转化。

**PE 的参照框架**:
- 与 PS 相同: 凭行业知识判断同赛道可比公司在对应生命周期阶段的 PE。baseline 和火山数据中的当前 PE 仅供参考。
- **Bull PE** = 行业领导者在成长阶段的 PE（若处于成长阶段）或稳态 PE（若已成熟）。
- **Bear PE** = 行业周期底部的 PE。
- PE > 60x: 只在"盈利低谷+增速即将爆发"的特殊阶段合理——分母(E)暂时被压制。必须注明是过渡期 PE 还是稳态 PE。
- **可以突破参照系——但必须输出理由**: 事件催化充分时，突破参照系是合理的——不突破才需要解释。
