...
2. 赋 CAGR 时——必须走事件数学，不走历史外推:
   a) 从事件锚点提取: 产能×利用率×价格 = 事件隐含收入。分别算各产品线。
   b) 用事件隐含收入 / 当前收入 = 隐含增长倍数。年化: CAGR = (倍数)^(1/年数)-1。
   c) 这个 CAGR 是你的基准。偏离基准超过 20% → 必须在 reasoning_trace 中逐条解释每条偏离原因（产能不及预期？价格到不了？利用率打折？）。不能只说"感觉太高"或"更保守"。
   d) 偏离理由必须是事件内部的具体信息（如"事件说 3000 吨但已公告产能仅 2000 吨"），不能是通用怀疑（如"高增速难以持续"）。
   e) 不允许因为当前收入低就压低事件推算的收入——事件已经改变了规模基线。
   f) 示例: 事件说 3000吨×150万/吨=45亿，当前4亿 → 隐含3年CAGR=(45/4)^(1/3)-1=124%。若你的base CAGR=55%，需具体解释: 多少来自利用率打折(如75%→33.8亿)? 多少来自价格打折(如120万→27亿)? 合计打折后CAGR=89%→给出理由。
...
- **收入硬校验**: 从事件锚点推算 3 年后收入（产能×价格×利用率），与 LLM-1 的 CAGR 隐含收入对比。差距 >30% → 必须在 change_log 中修正 CAGR 或 PS，不能只在 narrative 里说"更保守"。差距 >50% → 必须修正且不能只微调。
- 如果搜索证实了事件锚点（涨价确实在进行、产能确实在推进），LLM-1 无理由的大幅打折必须在 change_log 中修正
- 不能输出空表。至少要列出事件中最关键的 3 个量化锚点
