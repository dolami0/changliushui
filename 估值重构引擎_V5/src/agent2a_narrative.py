...
3. **分部经济学实质性分化** — 满足任一条:
   ...
   **数据优先级**: 火山搜索结果 > 最新季报数据 > 年报/H2数据。不要用全年的均值——用最近一个完整可获取周期的分部数据（如 Q1 财报的 Q1 单季毛利率，或火山搜索到的券商预测中最新季度的分部分析）。如果最新季度显示差距已突破阈值而全年数据还在阈值下，以最新季度为准。
...
- 叙事主锚业务收入占比 >80% → SOTP 加总与整体估值相差不大，拆分的额外误差不值得。但对于转型早期（新业务当前占比小但增速极高），即使叙事主锚业务 >80% 也应触发——因为旧锚框架无法合理定价新业务的非线性增长

**常见误判: "转型早期不适合SOTP" → 错。** 四条件满足就必须触发。转型早期≠不该拆——恰恰相反: 转型早期新业务占比小但增速极高,如果不拆,要么 (a)旧业务低增速拖垮PS→低估新业务,要么 (b)PS套到总收入上→高估旧业务。两者都是系统性错误。SOTP的存在就是为了防止这种污染——不管转型处于哪个阶段。

**边界裁决: 接近阈值就触发。** SOTP 的成本是引入两份估值误差,但不拆的成本是整个低毛利/亏损分部被系统性地赋予和主业一样的估值倍数——后者是更大的系统误差。当条件3卡在阈值附近时，选 SOTP。
