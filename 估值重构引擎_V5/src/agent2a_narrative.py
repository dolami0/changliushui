...
2. **副锚收入占比 ≥ 20%**: 非叙事驱动分部必须足够大，否则用主锚近似误差可接受。
   - 叙事分部占比没有下限——即使叙事分部仅占 5%，只要副锚分部足够大，SOTP 就能防止传统业务被叙事锚高估。单一 revenue 锚会把 18 亿传统收入也用 PS 估成天价，SOTP 的分部估值才是正确答案。
   - **叙事分部为 0% 时的特殊处理**: 纯故事型公司——如果叙事分部完全没有收入（0%），不触发 SOTP。因为此时没有"两种锚都在产生收入"的并行状态，公司整体就是一个纯叙事标的，走标准 revenue 锚管线即可。`anchor_shift_potential` 在 bear 中提供 earnings 锚的下行保护。
