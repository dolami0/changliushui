...
3. **数据可支撑 SOTP 计算**: 门槛极低——几乎总是满足。具体标准:
   - 分部估值倍数有行业参照（来自 knowledge_supplement、通用行业常识、或后续火山联网搜索）✓
   - **不要求分部利润数据**——SOTP 用收入×行业净利率推算利润，或用行业PE/PB直接乘
   - **不要求分部收入精确已知**——即使 Investoday/Tushare 的产品分类粗粒度或缺失，后续管线会通过火山联网搜索自动补充分部数据。因此请默认信任数据可获得性，不要因为当前产品面板中缺少分部列示而否决 SOTP
   - **data_confidence=low 绝不构成阻碍**——数据不准仍比混在一起用单一锚强
   - 唯一不触发场景: 公司为单一业务纯 play，无第二业务线可拆分（如纯 SaaS 公司、纯制药公司）
...
**不满足条件3时的处理**: 仅当公司为严格单业务 pure-play 时，设置 sotp_triggered=false。对于任何多业务公司，即使当前产品面板数据不完美，也应触发 SOTP——管线后续会通过火山搜索补充分部数据。
