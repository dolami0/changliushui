...
**触发条件——满足以下全部四条才设置 sotp_triggered=true:**
...
2. **叙事主锚分部收入占比 ≥ 20%**: 市场定价的核心业务必须有足够体量。SOTP 的"其他业务"用代码保守估值——如果叙事分部只占 10%，则 90% 的公司价值被粗糙公式决定，SOTP 的噪声远大于价值。
   - **反例**: 本川智能——叙事 CIPB 业务仅占 9.8%，90% 是传统 PCB。不触发 SOTP，走标准管线（单一 revenue 锚 + anchor_shift_potential 即可）
   - **正例**: 健信超导——超导产品占 74.4%，永磁占 20.7%，两边都有足够体量→触发
   - **叙事分部为 0% 时**: 纯故事型公司——没有"其他业务"需要分开估，走标准管线即可

3. **副锚收入占比 ≥ 20%**: 非叙事驱动分部也必须足够大，否则用主锚近似误差可接受。

4. **数据可支撑 SOTP 计算**: 条件4的门槛是"有数据可用"而非"数据精确"。具体标准:
...
**不满足条件2或3时的处理**: 叙事分部或副锚分部任一方体量不够→不触发 SOTP，走标准管线。单一锚 + anchor_shift_potential 已足够处理。

**不满足条件4时的 fallback**: 仅当完全无分部收入拆分时，设置 sotp_triggered=false，说明"无分部收入数据,以主锚为准"。
