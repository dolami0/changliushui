...
2. **副锚的锚类型**: 看该业务的定价指标是什么——市场实际用什么指标给它估值，就用什么锚。不要因为它是"存量"就默认 earnings——如果它也是看 PS 的，就用 revenue。
...
3. **分部经济学实质性分化** — 满足任一条:
   ...
   **数据优先级——事件 > 历史**: 产品结构表中的 TTM 增速/毛利率是**事件冲击前的过去状态**。事件可能已经改变了增速斜率（如设备订单从暴跌翻转为爆发）。判断条件 3 时，用事件素材中的**前瞻数据**（订单、产能规划、涨价幅度）来推断事件后的增速/毛利率分化——不以 TTM 历史数据为准。示例: 设备 TTM 增速 -52% 但事件说 Q1 订单 19.77 亿→前瞻增速 >60%；系统集成 TTM -19% 且事件未提及改善→仍为衰退。前瞻增速差 >80pp → 条件 3 满足，TTM 数据不构成反驳。
