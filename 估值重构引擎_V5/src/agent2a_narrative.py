...
```json
{
  "market_narrative": {
    ...
    "primary_anchor_evidence": "引用具体倍数和分位数据",
    "secondary_anchors": [...],
...
核心原则: **单一数据源不可信。锚判断必须从三个独立来源交叉验证。**

## 1a. 第一步: 估值倍数排除法（定量）

看哪个倍数在驱动市值。这是**必要条件**，不是充分条件。

| 盈利状态 | 主导倍数 | 推断 |
|---------|---------|------|
| 稳定盈利 (ROIC>8%) | PE 决定市值 | **利润锚候选** — 市场可能在定价盈利能力 |
| 微利/亏损 (ROIC 0-8%) | PS 高企而 PE 失真 | **收入锚候选** — 市场可能在定价收入增长和 TAM |
| 深度亏损 (净利润<0) | PE 无意义, PS/PB 决定 | **收入锚或资产锚候选** |
| 重资产+低ROE | PB 决定市值 | **资产锚候选** — 市场可能在定价净资产质量 |
| 创新药/biotech | PE/PS 均无效 | **管线锚候选** — 市场在定价药品管线 |

**关键原则**:
- 盈利企业 default 锚是利润——即使事件叙事围绕"收入/TAM"，PS 只是一个过渡指标，PE 才是终点
- 但如果公司处于"收入爆发但利润还没跟上"的阶段，锚是收入——市场在给增长定价而非当前利润
- 判断依据: PS 分位和 PE 分位的对比——哪个处于历史极端位置，哪个在驱动市值

## 1b. 第二步: 投资主题词频分析（定性——研报语料）

**仅完成 1a 不能下结论。** 必须检查用户消息中的"投资主题""事件推演""知识补充""行业研究"文本。

在这些文本中搜索锚定关键词，统计哪类词汇占主导:

| 锚类型 | 关键词信号 |
|--------|-----------|
| **earnings** | "盈利拐点""利润率提升""净利增速""ROIC改善""降本增效""经营杠杆" |
| **revenue** | "TAM""市场空间""收入增速""渗透率""国产替代""市占率提升""营收爆发""订单" |
| **asset** | "资产重估""隐蔽资产""NAV""清算价值""净资产""资产注入""土地/矿产" |
| **pipeline** | "管线""临床""获批""NDA/BLA""峰值销售""PoS""适应症""靶点" |
| **sotp** | "分部估值""业务分拆""新业务vs旧业务""转型""第二曲线" |

**判定规则**:
- 如果研报语料中 revenue 关键词密度 >> earnings 关键词密度，即使 ROIC>8%，市场仍在定价收入而非利润
- 如果研报语料中 earnings 关键词占主导，且 ROIC>8%，确认利润锚
- 如果研报语言与估值倍数推断矛盾——**以研报语言为准**，因为市场交易的是叙事而非财务指标

**市值-叙事一致性校验**:
- 对比用户消息中"估值倍数全矩阵"里给出的市值数据
- 小市值(<100亿) + TAM叙事 → 市场在定价远期预期，锚偏向收入/管线
- 大市值(>500亿) + 同样TAM叙事 → 市场在定价执行结果，锚偏向利润
- 同样的叙事在不同市值规模下含义完全不同

## 1c. 第三步: 产业调研参照（同行锚规范）

**检查"行业研究"和"知识补充"文本中是否提到了同行业、同阶段公司的估值实践。**

判定问题:
- 该行业/赛道的同行主要用什么锚？如果整个半导体材料行业都用 PS，个体公司 PE 再低也不能判为利润锚
- 该公司处于行业生命周期的哪个阶段？导入期→收入锚，成长期→利润锚，成熟期→资产锚
- 分析师覆盖该行业时默认的估值框架是什么？

**如果产业调研提供了明确的行业锚规范，其权重高于 1a 的倍数排除法。**

## 1d. 综合判定

三步结论的三角验证:
- 三步一致 → 高置信，直接确定
- 两步一致、一步矛盾 → 以 1b(研报语料) 和 1c(产业参照) 为优先，1a(倍数) 可能是滞后指标
- 三步全矛盾 → 说明市场定价混乱，标注为最可能的锚 + anchor_conflict 详细说明矛盾

在 `primary_anchor_evidence` 中必须引用:
- 具体倍数/分位数据（来自 1a）
- 研报语料中的锚定关键词（来自 1b）
- 产业调研中的同行锚参照（来自 1c，如有）

## 1e. 识别副锚（多业务/转型公司）

当存在以下情况时，标注 secondary_anchors:
- 公司有多个业务线，且不同业务处于不同生命周期
- 新业务快速增长但亏损，旧业务稳定盈利
- 市场可能对不同业务用不同的估值范式
