...
**把"定量定价工具"表格读三遍。** 这个表格告诉你: 在当前市值下，每种锚各自隐含了什么预期。

| 问题 | 分析 |
|------|------|
| 哪个锚的隐含预期最极端（最不合理）？ | 这个"离谱"不是错误——它是市场叙事的方向标 |
| 结合叙事，这个极端值意味着什么？ | 例: PE 241x 不一定意味着"市场在定价高增长"——如果是周期底部，PE虚高是因为分母被暂时压制 |
| 这个锚的隐含预期在叙事框架下是否合理？ | 例: 隐含3y CAGR 25%在行业增速范围内→revenue锚合理 |
...
**默认规则: 如果公司净利润 > 0，earnings 是默认锚。** 切换到其他锚需要积极证据。

切换到 revenue 锚必须**同时满足三个条件**:
1. **叙事语言**: 投资主题的核心变量是 TAM/渗透率/市占率/订单——不是"利润修复"或"扭亏"
2. **PE 确实不可用**: PE 当前值 > 60x 或为负，且不是因为周期底部利润压缩（利润较 3 年峰值下降 >70% 时 PE 虚高是周期效应，不是锚失效）
3. **行业范式**: 同赛道可比公司的市场定价确实以 PS 为核心指标（不是只有这一家 PS 高——而是整个行业都看 PS）

**科思股份示例判断**:
- 利润 1.01 亿 > 0 → 默认 earnings。举证责任在 revenue 方。
- 叙事在讲"利润弹性"（经营杠杆+毛利率修复）→ 不满足条件 1（叙事变量是利润率，不是 TAM）
- PE 56.9x 虚高，但利润较峰值下降 >80% → 是周期底部压缩，不满足条件 2
- 化妆品原料行业可比公司主要看 PE（华熙/珀莱雅等）→ 不满足条件 3
- **结论: earnings 锚成立。** 选 I(盈利正常化) 或 K(两阶段DCF)，两者都比 B(PS+TAM) 更能捕捉"利润从底部恢复"的核心叙事。

**切换到 asset 锚的条件**: (1) ROIC 持续 < 3% 且无改善路径，(2) PB < 1.5x 且资产以硬资产为主，(3) 叙事围绕资产重估/清算价值而非盈利改善。

**切换到 pipeline 锚的条件**: 仅限创新药/biotech，事件为临床试验数据/监管审批——不适用于化工/消费品行业的"新产品获批"。

### 2c. 产业语境

全行业是否在讲同一个故事？如果同赛道公司都在围绕同一叙事定价，个体公司不能例外。

### 2d. 锚冲突检测

如果叙事方向(2a)和市场实际定价指标(2b)不一致——标注 anchor_conflict，并解释原因。
