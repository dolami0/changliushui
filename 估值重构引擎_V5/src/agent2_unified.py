...
核心问题: **这家公司未来现金流的主要驱动变量是什么？**

### 2a. 盈利判断——这是唯一的锚判断

**规则: 净利润 > 0 → 锚 = earnings。没有例外。**

earnings 是默认锚，因为盈利公司的价值最终取决于它能赚多少钱。当前利润偏低——哪怕是周期底部——用 earnings 族内的模型处理即可（I 做正常化，K 做两阶段过渡），**不需要切换锚**。

**规则: 净利润 ≤ 0 → 进入 2b，选替代锚。**

只有不赚钱的公司才需要 earnings 以外的锚——因为 earnings 在此刻真的不存在。

### 2b. 替代锚（仅当 NP ≤ 0）

| 条件 | 锚 |
|------|-----|
| 收入高增长（>30% 或事件指向爆发）、TAM 扩张逻辑清晰、市场定价看 PS | revenue |
| 重资产（总资产/净资产 > 2x）、PB < 1.5x、核心价值在资产而非经营 | asset |
| 创新药/biotech、价值由临床管线驱动（成功率×峰值销售） | pipeline |
| 多业务、不同业务需要不同估值逻辑 | sotp |

**注意**: FDA 审批≠pipeline 锚。化妆品/化工/消费品的成分获批是 regulatory catalyst，锚仍然看公司本身（earnings 或 revenue），不是 pipeline。

### 2c. 数值验证——定价工具用于选模型，不用于改锚

定量定价工具告诉你：在当前锚下，市场隐含了什么样的预期。这个信息用于**第四步模型选择**——帮你在 earnings 族内选 A/K/G/I 的哪一个、在 revenue 锚下选 PS 的合理范围——而**不用于质疑 2a/2b 的锚判断**。

**误判警示**:
- PE 极高 + 公司盈利 → 这不代表"市场不看利润"——只是当前利润被周期压制。锚仍是 earnings。选 I（用周期中值利润）或 K（NOPAT 起点+DCF）。
- PS 极高 → 如果公司盈利，这不代表锚是 revenue。PS 高可能是因为收入也被周期压制了。
- 定价工具验证的是"在当前锚下，参数应该设多少"——不是"锚对不对"。
