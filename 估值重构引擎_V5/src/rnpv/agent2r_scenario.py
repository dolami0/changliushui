...
- 当前管线阶段/临床数据: PoS 的起点——Ph1/2/3 各有基准成功率，不能凭空假设。
- 已上市产品销售额/市场份额: 峰值销售的参照——不能超过 TAM × 合理份额。
- 可比药企 PE/PS 中位数: 成熟业务倍数的合理范围。
- 公司财务基线: 净现金/净负债、现有收入规模——估值的硬约束。

**事件锚（变数——参数的变化方向和幅度）**:
- 事件中的最新临床数据/FDA 进展/BD 合作/竞品格局变化——从 baseline 起点往哪个方向移、移多少。
- 按来源置信度打折: 临床数据读出/公司公告 <10%，专家纪要/券商预测 10-30%，管理层指引/峰值假设 20-40%。
- 打折必须显式标注: 锚点原文→来源→置信度→打折幅度→打折后数字→理由。

**两者的关系**: Baseline 是重力——PoS 不能超过 Ph3 的 60% 基准除非有突破性临床数据支撑。事件是推力——FDA 获批/BD 合作必须体现在 PoS 或峰值销售的上调中。参数的最终位置 = baseline 起点 + 事件推力 × 置信度打折。
