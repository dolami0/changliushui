...
找出投资主题和产业链报告中最脆弱的假设，设计红蓝对抗探针。每个探针是给火山搜索的精准指令。
...
```
攻击目标: 引用前序报告中的1-2句原话——指出你要挑战的具体判断。

搜索任务: 全面搜索可能推翻这个判断的反面证据。覆盖以下方向(列出5-8个具体搜索方向，每个是一个可搜到的数据点):
1. [具体指标A]的[最新数据]是否低于[预期/指引]
2. [行业报告/数据源]显示[趋势]是否反转
3. [财务指标]是否显示[恶化信号]
4. ...
N. [历史案例/可比公司]对比
...
1. **核心假设脆弱性**: 投资主题的if-then命题中，最脆弱的假设是哪个？
2. **两大失效测试**: (a)designed-out? (b)够material?
3. **利益博弈**: 谁最能挤压公司利润？
4. **外部冲击**: 政策/技术/地缘/宏观
5. **论点破裂条件**: 什么触发砍仓？

每个维度一个探针。不同探针禁止同质——每个必须攻击不同的假设、搜不同的数据。
...
{
  "probes": [
    {"name": "攻击估值", "task": "攻击目标: 引用N1: 'PE高达77.4倍...需要持续超预期增长来消化。'\\n搜索任务: 全面搜索反证...\\n1. AI新业务最新出货量是否低于预期\\n2. 行业报告是否显示需求放缓\\n...\\n输出要求: 列出反证(注明来源)/评估削弱程度(强中弱)/给出预警信号。"},
    ...
  ],
  "coverage_note": "..."
}
...
2. 互相冲突的结论 → 分两类处理:
   a) **事实性冲突**（如"竞争对手是否已量产""市占率是X%还是Y%"）→ 必须裁决。比较信息来源时效性和可靠性，选择更可信版本，说明裁决理由。无法裁决时标注"⚠️ **关键事实不确定** [需补充: 具体缺什么信息]"
   b) **判断性分歧**（如"护城河是强还是中"）→ 标注"⚠️ **判断存在分歧**"，保留双方观点
3. 全缺 → 标注"**[数据缺失]**"
...
1. 互相支持 → "**高置信度**"
2. 互相冲突 → 分两类:
   a) 事实性冲突（如"市占率是X%还是Y%"）→ 必须裁决。比较信息时效性和可靠性，选择更可信版本。无法裁决时标注"⚠️ **关键事实不确定** [需补充: 具体缺什么]"
   b) 判断性分歧（如"壁垒是强还是中"）→ 标注"⚠️ **判断存在分歧**"，保留双方观点
3. 全缺 → "**[数据缺失]**"
...
1. 互相支持 → "**高置信度**"
2. 互相冲突 → 分两类:
   a) 事实性冲突 → 必须裁决，比较来源时效性和可靠性
   b) 判断性分歧 → 标注"⚠️ **判断存在分歧**"，保留双方观点
...
**结论**: [一句话直接回答问题]
**最强证据**: [具体数字 + 必须标注来源URL或数据库名称。无来源的数字不得写入]
**最大缺口**: [如实写缺什么信息, 不要编造]
**一手来源**: [需要补的原始数据/报告名称]

⚠️ 铁律: 任何数字必须有源。无法追溯来源的数字 = 幻觉，不得输出。"""
...
def main():
    ...
    if n_probes == 0:
        print(f"[{FIELD_NAME}] [WARN] 探针设计失败: {coverage}", file=sys.stderr)
        print("")
        return
