...
1. 事件冲击比(45%)：暴露度 × 赔率杠杆。分两种类型评估。
   ...
   ① 暴露度得分（核心）— 该具体事件驱动的收入占总营收的比例，连续评估：

      ⚠️ 行业标签 ≠ 事件暴露。公司"属于AI行业"不等于"受益于AI数据集政策"。
      暴露度 = 该事件直接拉动的产品线收入 ÷ 公司总营收。不是"行业对口"的模糊判断。

      必须追问：该事件发布后，公司的哪条产品线、多大比例的收入会因此增长？
      例：某公司100%营收来自AI数据服务 → 若该政策只涉及多模态/具身智能数据集，
      而公司此类收入仅占15% → 暴露度 = 15%，而非 100%。
