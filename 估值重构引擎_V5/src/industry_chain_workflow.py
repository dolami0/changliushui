...
# TAM估算要求
TAM应覆盖该节点产品的全部可触达市场，不仅是事件直接拉动的部分。
例：液冷充电枪同时服务重卡超充 + 乘用车超充 + 储能快充，TAM应包含这三个场景，而非仅限重卡。
...
1. 事件冲击比(45%)：暴露度 × 赔率杠杆。分两种类型评估。
   ...
   ② 市值天花板 — 当前市值能否支撑 10x 上行（纯数学约束）：

      10x 目标市值 = 当前市值 × 10。该目标市值能否在节点 TAM 内合理实现？
      （节点 TAM 已由上游分析提供，直接使用。）

      <50亿:   10x→<500亿, 绝大多数节点TAM可容纳 → 几乎不折价(×0.9-1.0)
      50-100亿: 10x→500-1000亿, 需较大TAM → 轻微折价(×0.8-0.9)
      100-200亿: 10x→1000-2000亿, 需很大TAM → 中等折价(×0.6-0.8)
      200-300亿: 10x→2000-3000亿, 需极大TAM → 显著折价(×0.4-0.6)
