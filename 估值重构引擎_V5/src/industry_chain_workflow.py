...
class IndustryChainWorkflow:
    ...
    def _search_nodes(self, chain: dict, event_summary: str = "") -> str:
        ...
        for n in nodes:
            ...
            q = (
                f"搜索「{name}」产业链节点的A股公司。优先市值 20-300 亿范围内的标的（市值>300亿的公司会在后续环节被剔除）。\n\n"
                f"背景事件：{evt}\n"
                f"该节点胜出公司的特征：{what}\n\n"
                f"输出要求（每家公司一个条目）：\n"
                f"【公司名+代码】主营业务与节点的匹配说明\n"
                f"【竞争壁垒】核心壁垒+市场份额\n"
                f"【业绩与催化】近期业绩趋势+催化事件\n\n"
                f"优先券商研报和公告中明确点名属于该节点的公司，至少列出3家。"
            )
            queries.append(q)
    ...
    def _fetch_stock_intel(self, stock_code: str, stock_name: str,
        ...
        query = (
            ...
            f"⚠️ 不要搜索市值/PE/PB/估值数据——这些由tushare实时行情提供，网络搜索的估值数据已过时。\n"
