...
class IndustryChainWorkflow:
    ...
    def _search_nodes(self, chain: dict, event_summary: str = "") -> str:
        ...
        for n in nodes:
            ...
            q = (
                f"搜索「{name}」产业链节点的A股公司。\n\n"
                f"背景事件：{evt}\n"
                f"筛选标准：{what}\n\n"
                f"⚠️ 市值约束：后续评分环节会剔除非所有市值>300亿的公司。请优先搜索和列出市值 20-300 亿范围内的公司。若某节点的龙头企业市值均>300亿，必须在列出龙头的同时，特别搜索并列出 2-3 家市值<300亿但在该节点有显著业务暴露的公司（如'二线焦煤企业''纯正矿安标的'等）。\n\n"
                ...
                f"- 至少列出 3 家，最多 5 家，其中至少 2 家市值 < 300 亿"
            )
            queries.append(q)
