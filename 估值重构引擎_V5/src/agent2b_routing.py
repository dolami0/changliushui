...
**B (PS+TAM)**: 收入阶段的拐点模型。当投资主题是"收入先爆发、利润后跟"——涨价、产能爬坡、产品升级→收入快速增长→规模效应释放→利润率非线性跃升——B 用收入增速和 PS 刻画这个路径，不需要当前利润。PE/DCF 要求利润做分母，在拐点前都是失效的。B 不需要"转型例外"——它本来就是为收入阶段的拐点公司设计的。**转型例外**: 若2a的叙事诊断满足以下3条,允许盈利企业使用B:
...
**K (两阶段DCF)**: **仅限earnings_multiples族内选择**。适用于当前盈利、高增长(>25%)、行业终局清晰(5年后增速必然回落)、且**NOPAT起点可支撑DCF**的标的。与A/G/I的区别:
  ...
  **不选K的场景**:
    ...
    - primary_anchor=revenue→此标的属于revenue_multiples族,应选B; K仅限earnings_multiples族内使用
	  **注意**: PS不是"成长溢价"——它是亏损/微利公司唯一可用的估值指标。当ROIC<0、净利润为负时，PE和DCF都失效（分母不存在），PS是市场实际在用的定价语言（看当前18.1x PS即知）。深度亏损≠不该用PS——恰恰相反，亏损公司只能用PS。
