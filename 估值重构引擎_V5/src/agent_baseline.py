...
|---------|------------|----------------|
| 投资主题 (investment_theme) | 五(投资主线)、四(增长轨迹) | 核心叙事、估值锚、关键假设、**变量弹性系数**（如"每+1000片→+X亿营收"→写入量化锚点）、**砍仓触发条件**（量化到具体数字→写入脆弱点分析）、开篇隐喻（"卖铲人""军火商"→用于一句话穿透）、核心变量及对冲关系、预期差分析（市场定价了什么漏了什么） |
...
- 护城河: 成本优势/技术壁垒/客户粘性/牌照壁垒的评级和依据。**不要笼统说"壁垒高"——引用产业链报告中的三层拆解（技术/资本/认证）分别说明**
- 客户/供应商集中度: 是否有单点依赖风险
