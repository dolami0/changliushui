...
两部分素材:
- **事件本身**（原始事件、事件研判、背景知识）：产能、价格、供需缺口的当前数据——赋增长率/利润率的首要依据。
- **个股路线**（投资主题、发展推演、催化节点、逆向风险）：公司的既定轨迹。逆向风险约束 bear 证伪路径，催化节点补充里程碑时间线。
...
**各分部 PE/PS/PB 锚定 — baseline 已提供参照系:**

你在赋各分部的 target_ps / pe_target / target_pb 时，参照系不在火山数据中——在 baseline 里:
- 「产业位置」维度: 各业务线对应的可比公司+市场地位
- 「量化锚点」维度: 可比 PE/PS/PB 中位数 + 历史 PE band

**分部门槛规则（使用 baseline 数据校准）:**
- **事件催动分部**: PE/PS 可超出可比中位数但须在行业领导者的历史峰值之内。如果 baseline 中没有一家可比公司在稳态期交易到这个水平 → 你的假设缺乏市场支撑。
- **非事件催动分部**: PE/PS ≤ 可比中位数。直接从 baseline 量化锚点取数。
- **Bear 分部门槛**: 按上文 Bear 地板表执行。由 baseline 脆弱点判断哪个分部最脆。

**赋完各分部参数后做 baseline 交叉校验:**
- 各分部 base PE/PS 与 baseline 可比中位数的偏差 → 事件催动分部可以有溢价但要写理由，非事件催动分部超出需解释
- 各分部 bull PE/PS 是否超过 baseline 参照系中最好公司在非泡沫期的交易峰值
- SOTP 加总 target_mcap 与公司当前市值的差距方向是否与 2a 的计价程度一致（高估→加总应低于市值，低估→高于市值）
...
- **锚定方法**: 可比公司 PS/PE 的当前时点值可能受板块情绪影响——结合基线数据和历史稳态区间判断。
...
**PE 的参照框架**:
- PE 的锚定逻辑与 PS 相同: baseline「量化锚点」中的可比 PE 中位数是锚定起点。火山数据仅供参考。
- **Bull PE** = 行业领导者在稳态下的 PE。baseline 参照系中盈利质量最高的可比公司。
- **Bear PE** = 行业周期底部的 PE。baseline 历史 PE band 的底部区域。故事证伪不等于公司归零。
...
**其他业务** (is_primary=false): 事件催化剂只驱动叙事主线，不影响传统业务。因此其他业务不需要推演三情景——只需要判断它的合理估值是多少（一组 base 参数），bear/base/bull 三个情景都用这同一个估值。具体来说：
  ...
  - PE/PS/PB 的锚定法则与主锚分部相同: baseline「量化锚点」中的可比中位数是锚定起点。火山数据中的当前倍数仅供参考，行业间估值中枢差异已在 baseline 中体现。
