...
- 财务基线: 当前 ROIC/毛利率/净利率/营收——各分部 segment_revenue_yi 和 segment_net_margin_pct 的**推演起点**。
- 产业位置: 每个分部的可比公司 PE/PS/PB 中位数——各分部倍数的**合理范围**。超出范围 → 必须有"突破论证"。
- 量化锚点: 当前产能/价格/市占率——事件冲击前的**硬数字**。
- 脆弱点: 什么假设最容易崩塌——约束 bear 的下跌空间和 bull 的上限。

**事件锚（变数——参数的变化方向和幅度）**:
- 事件中的新价格/新产能/新客户——从 baseline 起点往哪个方向移、移多少。
- 按来源置信度打折: 公司公告 <10%，专家纪要 10-30%，分析师测算 20-40%。
- 打折必须显式标注: 锚点原文→来源→置信度→打折幅度→打折后数字→理由。

**两者的关系**: Baseline 是重力——参数不能飞到物理不可能的水平。事件是推力——参数不能停在 baseline 不动。参数的最终位置 = baseline 起点 + 事件推力 × 置信度打折。
