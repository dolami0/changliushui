...
**三者的关系**: 地图告诉你"从哪出发"，事件告诉你"往哪走、走多远"。分部参数 = 理解起点（baseline）+ 事件冲击（event）。

**数据信任层级——时间戳 > 一切**: 事件是唯一最新情报。事件中的产能/价格/利用率/订单是当前情形的基准——baseline 和财报反映的是事件冲击前的过去状态，不可替代事件数据。当事件数据与财报冲突时，以事件为准。

**⚠️ 事件量化锚点——赋值前必须提取并回应:**

读完事件素材后，从三个来源提取量化锚点: ①事件原文 ②背景知识 ③baseline。在 reasoning_trace 的"清单项1"末尾列出:
```
[事件量化锚点 — 含来源标注]
...
2. **读他们的数**: 这些可比公司当前交易的 PE/PS 是多少？它们在对应生命周期阶段交易的区间是多少？
3. **对标赋参**: 你的 bear/base/bull PE/PS 反映公司**当前生命周期位置**——高增长阶段享受成长溢价，成熟阶段回归稳态。可以有溢价/折价,但必须有理由。

**PS 的参照框架**:
- **锚定方法**: 凭行业知识判断同细分赛道的 A 股公司在对应生命周期阶段的估值水平。baseline 量化锚点和火山数据中的可比公司 PS/PE 仅供参考。
- **Bull PS** = 行业领导者在成长阶段的 PS（若处于成长阶段）或稳态 PS（若已成熟）。不是泡沫峰值。
- **Base PS** = 反映公司**当前生命周期位置**的合理PS——不是稳态PS。CAGR>30%、新技术刚验证、订单爆发→取成长阶段对应水平。事件催化（验证突破、涨价东风、订单翻倍）必须体现在 PS 溢价中。
- **Bear PS** = commoditized 参与者或周期底部的 PS。不是危机恐慌低点——是"故事证伪后,市场在正常情况下持续交易该股票的底部区间"。
- **可以突破参照系——但必须输出理由**: 事件催化充分时，突破参照系是合理的——不突破才需要解释为什么催化没转化成溢价。

**PE 的参照框架**:
- PE 的锚定逻辑与 PS 相同: 对应生命周期阶段，非机械对标稳态。
- **Bull PE** = 行业领导者在成长阶段或稳态的 PE（取决于生命周期位置）。
- **Bear PE** = 行业周期底部的 PE。故事证伪不等于公司归零。
- PE > 60x: 只在"盈利低谷+增速即将爆发"的特殊阶段合理——分母(E)暂时被压制。
...
你的 PE/PS/PB 锚定的是【可比公司对应生命周期阶段的估值水平】。当前市值只是一个事实标签，不是你赋参数的靶子。赋完参数后不要心算估值——代码会算。
