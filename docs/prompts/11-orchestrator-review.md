...
- [sysprompt](#sysprompt) — `估值重构引擎_V5/src/orchestrator.py`
- [user_msg](#user_msg) — `估值重构引擎_V5/src/review/dream_loop.py`
- [INTROSPECTOR_SYSTEM](#introspector_system) — `估值重构引擎_V5/src/review/prompts.py`
- [DREAM_LOOP_SYSTEM](#dream_loop_system) — `估值重构引擎_V5/src/review/prompts.py`
- [Introspector user_message 模板](#introspector-user_message-模板) — `估值重构引擎_V5/src/review/introspector.py`

---
<a id="sysprompt"></a>
## sysprompt

- **源码**: `估值重构引擎_V5/src/orchestrator.py`  · 行 198-213
- **符号**: `sysprompt`
- **管线阶段**: 管线 C · Orchestrator 数据补充 query
- **类型**: f-string · LLM
- **说明**: f-string 动态模板
...
```text
你是估值数据补充助手。当前管线对 {stock_name}({stock_code}) 做估值推演。
...
- **源码**: `估值重构引擎_V5/src/review/dream_loop.py`  · 行 227-231
- **符号**: `user_msg`
- **管线阶段**: 管线后 · Introspector / Dream Loop
- **类型**: f-string · LLM
- **说明**: f-string 动态模板
...
````text
以下是过去 {len(logs)} 次管线运行的审阅记录（已压缩关键字段）。
...
<a id="introspector_system"></a>
## INTROSPECTOR_SYSTEM

- **源码**: `估值重构引擎_V5/src/review/prompts.py`  · 行 18-145
- **符号**: `INTROSPECTOR_SYSTEM`
- **管线阶段**: 管线后 · Introspector / Dream Loop
- **类型**: str · LLM
...
<a id="dream_loop_system"></a>
## DREAM_LOOP_SYSTEM

- **源码**: `估值重构引擎_V5/src/review/prompts.py`  · 行 152-182
- **符号**: `DREAM_LOOP_SYSTEM`
- **管线阶段**: 管线后 · Introspector / Dream Loop
- **类型**: str · LLM
...
<a id="introspector-user_message-模板"></a>
## Introspector user_message 模板

- **源码**: `估值重构引擎_V5/src/review/introspector.py`  · 行 _build_user_message L92+
- **符号**: `Introspector user_message 模板`
- **管线阶段**: 管线后 · Introspector
- **类型**: template · LLM
- **动态注入字段**: stock_name, stock_code, 5 份预研字段（各截断 8000 字）, baseline_report, agent2_result 摘要, agent3_result 情景摘要
- **说明**: 完整拼接逻辑在函数内，非单一三引号常量。

### 提示词正文

```text
# 审阅对象: {stock_name} ({stock_code})
...
{investment_theme truncated to 8000}

### N2 行业研究 ({len} 字)

{industry_expert_research truncated to 8000}

### N5 事件推演 ({len} 字)

{event_deduction truncated to 8000}

### N3 逆向推演 ({len} 字)

{adversarial_thinking truncated to 8000}

### N4 催化日历 ({len} 字)

{future truncated to 8000}

（后续 L2 Baseline / L3 Agent-2 / L4 Agent-3 摘要由 _extract_* 压缩注入，见 introspector.py _build_user_message）
```
